...
## provided by open source R packages, RevoScaleR and MicrosoftRML libraries
## Author: Microsoft
####################################################################################################
...
####################################################################################################
## Gradient tree boosting with mxFastForest on SQL data source
####################################################################################################
...
system.time({
mx_fastforest_model <- mxFastForest(formula = formula,
                                data = train_table,
                                type = "binary",
                                numTrees = 8,
                                numLeaves = 20,
...
mx_fastforest_model
summary(mx_fastforest_model)

## Score model
rxSetComputeContext('local')
predictions <- mxPredict(modelObject = mx_fastforest_model,
                         data = test_table,
                         extraVarsToWrite = c("customerid", "churn"),
                         overwrite = TRUE)
head(predictions)
pred_df <- predictions[, -4]
names(pred_df) <- c("customerid", "churn", "FastForest_Prediction", "FastForest_Probability")
head(pred_df)

## Evaluate model
mx_fastforest_metrics <- evaluateModel(data = pred_df,
                                    observed = "churn",
                                    predicted = "FastForest_Prediction")
mx_fastforest_metrics

rxrocCurve(data = pred_df,
                         observed = "churn",
                         predicted = "FastForest_Probability")

####################################################################################################
## Gradient tree boosting with mxFastTree on SQL data source
####################################################################################################
...
head(predictions)
pred_df <- predictions[, -4]
names(pred_df) <- c("customerid", "churn", "FastTree_Prediction", "FastTree_Probability")
head(pred_df)

## Evaluate model
mx_fasttree_metrics <- evaluateModel(data = pred_df,
                                    observed = "churn",
