...
BEGIN
    ...
    END ELSE BEGIN -- if in-memory can be created
		...
		BEGIN TRY
			...
			BEGIN
        /* 
        -- In original version filename was hard coded. Updated to use a version
        -- so the filename doesn't matter.
				DECLARE @SQLDataFolder nvarchar(max) = (SELECT SUBSTRING(df.physical_name, 1, CHARINDEX(N'WideWorldImportersDW.mdf', df.physical_name, 1) - 1)
				                                        ...
				                                        WHERE df.file_id = 1);
        */
        -- Updated version RCCain 2017-01-23
				DECLARE @SQLDataFolder nvarchar(max) = (SELECT LEFT(df.physical_name,LEN(df.physical_name) - charindex('\',reverse(df.physical_name),1) + 1)
				                                          FROM sys.database_files AS df
				                                         WHERE df.file_id = 1);
